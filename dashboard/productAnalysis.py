from productReceipt.models import Receipt_No as PRN, Item as PRI
import datetime
...
#week analysis
    ...
    #year analysis
    jan_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 1, created_at__year=x.year)
    jan_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 1, receipt_no__created_at__year=x.year)
    total_sales_jan = 0
    for jan_item in jan_items:
        total_sales_jan += jan_item.get_sub_total()

    feb_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 2, created_at__year=x.year)
    feb_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 2, receipt_no__created_at__year=x.year)
    total_sales_feb = 0
    for feb_item in feb_items:
        total_sales_feb += feb_item.get_sub_total()

    mar_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 3, created_at__year=x.year)
    mar_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 3, receipt_no__created_at__year=x.year)
    total_sales_mar = 0
    for mar_item in mar_items:
        total_sales_mar += mar_item.get_sub_total()

    apr_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 4, created_at__year=x.year)
    apr_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 4, receipt_no__created_at__year=x.year)
    total_sales_apr = 0
    ...
    may_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 5, created_at__year=x.year)
    may_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 5, receipt_no__created_at__year=x.year)
    total_sales_may = 0
    for may_item in may_items:
        total_sales_may += may_item.get_sub_total()
    
    jun_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 6, created_at__year=x.year)
    jun_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 6, receipt_no__created_at__year=x.year)
    total_sales_jun = 0
    for jun_item in jun_items:
        total_sales_jun += jun_item.get_sub_total()

    jul_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 7, created_at__year=x.year)
    jul_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 7, receipt_no__created_at__year=x.year)
    total_sales_jul = 0
    for jul_item in jul_items:
        total_sales_jul += jul_item.get_sub_total()

    #august
    aug_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 8, created_at__year=x.year)
    aug_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 8, receipt_no__created_at__year=x.year)
    total_sales_aug = 0
    for aug_item in aug_items:
        total_sales_aug += aug_item.get_sub_total()
    
    #september
    sep_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 9, created_at__year=x.year)
    sep_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 9, receipt_no__created_at__year=x.year)
    total_sales_sep = 0
    for sep_item in sep_items:
        total_sales_sep += sep_item.get_sub_total()

    #october
    oct_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 10, created_at__year=x.year)
    oct_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 10, receipt_no__created_at__year=x.year)
    total_sales_oct = 0
    for oct_item in oct_items:
        total_sales_oct += oct_item.get_sub_total()
    
    #november
    nov_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 11, created_at__year=x.year)
    nov_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 11, receipt_no__created_at__year=x.year)
    total_sales_nov = 0
    for nov_item in nov_items:
        total_sales_nov += nov_item.get_sub_total()

    dec_data = PRN.objects.filter(
        user_id=get_current_authenticated_user().id, created_at__month = 12, created_at__year=x.year)
    dec_items = PRI.objects.filter(receipt_no__user_id=get_current_authenticated_user().id, receipt_no__created_at__month = 12, receipt_no__created_at__year=x.year)
    total_sales_dec = 0
